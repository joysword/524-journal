# The Application of Visualization Systems in Urban Planning related tasks
...
Online mapping APIs are new tools that emerged only in recent years. The advantages of them are:

1. free for use. Most APIs are provided to any user for free, while some are not, or have limited functionality in free version. For example, ArcGIS has a Javascript API, which is not as powerful as its desktop mapping software ArcGIS for Desktop and its mapping servicer software ArcGIS for Server, but includes most of the frequently used functionality.

2. easy-to-use. Most APIs are well documented and organized.

3. scalability. Some of online mapping APIs, such as Google Maps API, Bing Maps API, are providing limited version for free use, and provided also non-free versions for users with special needs.
